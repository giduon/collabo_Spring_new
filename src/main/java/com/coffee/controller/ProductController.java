...
public class ProductController {
    ...
    public ResponseEntity<?> insert(@RequestBody Product product){
        ...
        // 데이터 베이스와 ㅣ미지 경로에 저장될 이미지의 이름
        String imageFileName = "product_" + System.currentTimeMillis() + ".png";

        //String 클래스 공부 : endsWith(), split() 메소드

        // 폴더 구분자가 제대로 설정 되어 있으면 그대로 사용합니다.
        // 그렇지 않으면, 폴더 구분자를 붙여 줍니다.
        // File.separator : 폴더 구분자를 의미하며, 리북스는 /, 윈도우는 \입니다.
    ...
    @GetMapping("/update/{id}") //상품의id정보를 이용하여 해당 상품 Bean 객체를 반환해 줍니다.
...
//        System.out.println(updatedProduct);
        ...
        if (findProduct.isEmpty()){ // 상품이 존재하지 않으면 404응답 반환
            ...
            try {
                // 이전 이미지 객체에 새ㅗ운 이미지 객체 정보를 업데이트 합니다.
...
//                savedProduct.setInputdate(LocalDate.now());
                ...
                //이미지가 의미있는 문자열로 되어있고, Base64 인코딩 형식이면 이미지 이름을 변경합니다.
            ...
            }catch (Exception err){ // 오류 발생시 500 응답코드 반환
                return ResponseEntity
                        .status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body(Map.of("message", err.getMessage(), "error", "Error update failed"));
            }
        }
    }

    //Base64 인코딩 문자열을 변환하여 이미지로 만들고, 저장해주는 메소드 입니다.
    private String savedProductImage(@NotBlank(message = "이미지는 필수 입력 사항입니다.") String base64Image) {
        ...
        // base64Image : JaveScript FileReader API에 만들어진 이미지입지다.
        byte[] decodedImage = Base64.getDecoder().decode(base64Image.split(",")[1]);

        try{ //FileOutputStream 는 바이트 파일을 처리해주는 자바의 Stream 클래스
            FileOutputStream fos = new FileOutputStream(imageFile);
