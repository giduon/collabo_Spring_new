...
public class Cart {
    ...
    @OneToOne(fetch = FetchType.LAZY) // 일대일 연관 관계 메핑, 지연 로딩(Lazy)
    @JoinColumn(name = "member_id")
    private Member member;

    // Cart 1개에는 여러 개의 '카트 상품'들이 담겨질 수 있으므로, 컬렉션 형태로 작성해야 합니다.
    // CascadeType.ALL : 카트 정보에 변경/수정/삭제 드의 변동 사항이 발생하면, 모두 '카트 상품'에 전부 반영 시켜 주세요
    // CascadeType 와 관련하여 데이터 베이스의 on delete set null, on delete cascade 공부
